...
class module_controller extends ctrl_module
{
    ...
    static function WriteCronFile()
    {
        ...
        if (sys_versions::ShowOSPlatformVersion() != "Windows") {
            $line .= 'SHELL=/bin/bash' . PHP_EOL;
            $line .= 'PATH=/sbin:/bin:/usr/sbin:/usr/bin' . PHP_EOL;
            $line .= 'HOME=/' . PHP_EOL;
            $line .= PHP_EOL;
        }
        ...
        $line .= "# File automatically generated by Sentora " . sys_versions::ShowSentoraVersion() . PHP_EOL;
        if (sys_versions::ShowOSPlatformVersion() == "Windows") {
            $line .= "# Cron Debug infomation can be found in file C:\WINDOWS\System32\crontab.txt " . PHP_EOL;
            $line .= "#################################################################################" . PHP_EOL;
            $line .= "" . ctrl_options::GetSystemOption('daemon_timing') . " " . $restrictinfos . ctrl_options::GetSystemOption('daemon_exer') . PHP_EOL;
        }
        $line .= "#################################################################################" . PHP_EOL;
        $line .= "# NEVER MANUALLY REMOVE OR EDIT ANY OF THE CRON ENTRIES FROM THIS FILE,          " . PHP_EOL;
        $line .= "#  -> USE SENTORA INSTEAD! (Menu -> Advanced -> Cron Manager)                    " . PHP_EOL;
        $line .= "#################################################################################" . PHP_EOL;
        ...
        if ($numrows->fetchColumn() <> 0) {
            ...
            while ($rowcron = $sql->fetch()) {
                ...
                if ($rowclient && $rowclient['ac_enabled_in'] <> 0) {
                    ...
                    $line .= $rowcron['ct_timing_vc'] . " " . $restrictinfos . $rowcron['ct_fullpath_vc'] . PHP_EOL;
